...
<style>
    ...
    * { font-family: 'DM Sans', -apple-system, BlinkMacSystemFont, sans-serif; }
    ...
    .presenter-badge {
        background: linear-gradient(135deg, #E85A4F 0%, #F07B73 100%);
        color: white;
        padding: 4px 10px;
        border-radius: 20px;
        font-size: 10px;
    ...
    .presenter-highlight {
        background: linear-gradient(135deg, rgba(232, 90, 79, 0.15) 0%, rgba(232, 90, 79, 0.08) 100%);
    ...
    body.mobile-mode-active .presenter-panel {
        padding: 12px;
        margin-bottom: 12px;
    }
    
    body.mobile-mode-active .presenter-title {
        font-size: 13px;
    }
    
    body.mobile-mode-active .talking-point-text {
        font-size: 11px;
    }
...
DEMO_SCRIPT = {
    0: {
        ...
        "talking_points": [
            "Customer is logged in as Alex Morgan, a PLATINUM member with 15% discount",
            "7 AI agents are standing by - each handles a specific commerce function",
            "Try a conversational query like 'Help me plan a birthday party for a 5 year old'",
            "Watch how Cortex Search understands intent, not just keywords"
        ],
        "highlight": "The AI understands context - birthday + age = relevant toys & party supplies"
    },
    1: {
        "title": "Product Discovery",
        "talking_points": [
            "Cortex Search Service returns semantically relevant products",
            "Notice: NO planters, garden tools, or adult items for a kids party",
            "Each product shows real-time inventory from nearest distribution center",
            "Click a product to see the full agent orchestration"
        ],
        "highlight": "Agent 1 (Demand Sensing) and Agent 2 (Customer Intelligence) now active"
    },
    2: {
        "title": "Product Details & Pricing",
        "talking_points": [
            "Dynamic pricing calculated by the Pricing Agent in real-time",
            "Inventory Optimizer checks ATP across all 5 distribution centers",
            "Customer Intelligence applies the PLATINUM 15% discount",
            "Cortex Complete generates personalized recommendation"
        ],
        "highlight": "Agents 3 (Inventory) and 4 (Pricing) orchestrating the response"
    },
    3: {
        "title": "Cart & Checkout",
        "talking_points": [
            "Multiple items can be added to cart",
            "Each item maintains its agent context and pricing",
            "Checkout triggers the final 3 agents simultaneously",
            "Watch the agent panel - all 7 will activate"
        ],
        "highlight": "Ready to demonstrate full agent orchestration at checkout"
    },
    4: {
        "title": "Fulfillment & Logistics",
        "talking_points": [
            "Fulfillment Orchestrator selects optimal shipping location",
            "Logistics Optimizer calculates delivery route and ETA",
            "Supplier Collaboration triggers auto-replenishment PO",
            "All transactions recorded to Snowflake in real-time"
        ],
        "highlight": "Agents 5, 6, and 7 complete the end-to-end flow"
    },
    5: {
        "title": "Order Complete",
        "talking_points": [
            "All 7 agents executed from discovery to delivery",
            "Complete audit trail in Snowflake NOVAMART_RETAIL database",
            "Auto-replenishment ensures inventory stays optimal",
            "Ready to demo again with different scenario"
        ],
        "highlight": "Full agentic commerce flow in ~60 seconds"
    }
}
...
col_presenter_toggle, col_presenter_spacer = st.columns([1, 4])
with col_presenter_toggle:
    presenter_mode = st.toggle("Presenter Mode", value=st.session_state.presenter_mode, key="presenter_toggle")
    if presenter_mode != st.session_state.presenter_mode:
        st.session_state.presenter_mode = presenter_mode
        st.rerun()

if st.session_state.presenter_mode:
    current_step = min(st.session_state.flow_step, 5)
    script = DEMO_SCRIPT[current_step]
    ...
    <div class="presenter-panel">
        <div class="presenter-header">
            <div class="presenter-title">
                ...
                <span class="presenter-badge">Step {current_step + 1}/6</span>
            </div>
            <div class="presenter-step">
                {''.join([f'<span class="presenter-step-num {"active" if i == current_step else ""}">{i+1}</span>' for i in range(6)])}
            </div>
        </div>
        <div style="font-size: 18px; font-weight: 700; color: #F1F5F9; margin-bottom: 12px;">{script['title']}</div>
        <div class="talking-points">
            {''.join([f'<div class="talking-point"><span class="talking-point-icon">▸</span><span class="talking-point-text">{point}</span></div>' for point in script['talking_points']])}
        </div>
        <div class="presenter-highlight">
            <div class="presenter-highlight-label">Key Message</div>
            <div class="presenter-highlight-text">{script['highlight']}</div>
        </div>
    </div>
    """, unsafe_allow_html=True)

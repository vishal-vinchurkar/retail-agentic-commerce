...
st.set_page_config(
    page_title="NovaMart · Agentic Retail Intelligence",
    page_icon="✦",
    layout="wide",
    initial_sidebar_state="collapsed"
)

def get_theme_css(dark=True):
    if dark:
        bg = "#0B1120"; card_bg = "rgba(19,27,46,0.85)"; card_border = "rgba(255,255,255,0.08)"
        text = "#E2E8F0"; text_muted = "#94A3B8"; text_faint = "#8896AB"
        surface = "rgba(30,41,59,0.7)"; header_bg = "rgba(11,17,32,0.88)"
        input_bg = "rgba(30,41,59,0.6)"; input_border = "rgba(255,255,255,0.2)"
        hover_glow = "rgba(232,90,79,0.2)"; glass_border = "rgba(255,255,255,0.06)"
    else:
        bg = "#FFFBF8"; card_bg = "rgba(254,253,251,0.95)"; card_border = "#E7E5E4"
        text = "#1C1917"; text_muted = "#57534E"; text_faint = "#A8A29E"
        surface = "#F5F5F4"; header_bg = "rgba(255,251,248,0.92)"
        input_bg = "#FEFDFB"; input_border = "#D6D3D1"
        hover_glow = "rgba(232,90,79,0.12)"; glass_border = "#E7E5E4"
    return bg, card_bg, card_border, text, text_muted, text_faint, surface, header_bg, input_bg, input_border, hover_glow, glass_border

is_dark = st.session_state.get("theme_mode", "dark") == "dark"
bg, card_bg, card_border, text_color, text_muted, text_faint, surface, header_bg, input_bg, input_border, hover_glow, glass_border = get_theme_css(is_dark)
...
<style>
    ...
    :root {{
        --primary: #E85A4F;
        --primary-light: {('#2A1A18' if is_dark else '#FFF5F4')};
        --primary-dark: #D94A3F;
        --primary-gradient: linear-gradient(135deg, #E85A4F 0%, #E96D64 50%, #F07B73 100%);
        --accent: #8E6C5A;
        --accent-light: {('#1E1612' if is_dark else '#F5F0ED')};
        --success: #34D399;
        --success-light: {('rgba(52,211,153,0.12)' if is_dark else '#E8F5F0')};
        --warning: #FBBF24;
        --warning-light: {('rgba(251,191,36,0.12)' if is_dark else '#FEF8EC')};
        --danger: #F87171;
        --danger-light: {('rgba(248,113,113,0.12)' if is_dark else '#FEF2F1')};
        --bg: {bg};
        --card-bg: {card_bg};
    ...
    * {{ font-family: 'DM Sans', -apple-system, BlinkMacSystemFont, sans-serif; }}
    ...
    @keyframes blink {{ 0%,100% {{ border-color: transparent; }} 50% {{ border-color: var(--success); }} }}
    @keyframes confettiFall {{
        0% {{ transform: translateY(-100vh) rotate(0deg); opacity: 1; }}
        100% {{ transform: translateY(100vh) rotate(720deg); opacity: 0; }}
    }}
    @keyframes glowPulse {{
        0%, 100% {{ box-shadow: 0 0 4px rgba(232,90,79,0.3); }}
        50% {{ box-shadow: 0 0 20px rgba(232,90,79,0.6); }}
    }}
    ...
    .boot-overlay {{
        ...
        background: #0B1120; z-index: 99999;
    ...
    .boot-line {{
        font-size: 14px; color: #64748B; font-family: 'JetBrains Mono', monospace;
        opacity: 0; animation: bootFadeIn 0.5s ease forwards;
    }}
    .boot-line.ready {{
        color: var(--success); font-weight: 600;
    }}
    .boot-progress {{
        width: 200px; height: 3px; background: #1E293B; border-radius: 2px;
        margin-top: 16px; overflow: hidden;
    }}
    .boot-progress-bar {{
        height: 100%; background: var(--primary-gradient); border-radius: 2px;
    ...
    /* ── Floating Snowflake Badge ── */
    .floating-badge {{
        position: fixed; bottom: 24px; right: 24px; z-index: 9999;
        display: flex; align-items: center; gap: 8px;
        padding: 10px 18px;
        background: {('rgba(19,27,46,0.9)' if is_dark else 'rgba(255,255,255,0.95)')};
        backdrop-filter: blur(12px);
        border: 1px solid {('rgba(41,181,232,0.3)' if is_dark else 'rgba(41,181,232,0.2)')};
        border-radius: 24px;
        box-shadow: 0 4px 20px rgba(41,181,232,0.15);
    ...
    /* ── Protocol Indicator Strip ── */
    .protocol-strip {{
        display: flex; align-items: center; justify-content: center; gap: 8px;
        padding: 10px 20px; margin: 0 -2rem;
        background: {('rgba(15,23,42,0.6)' if is_dark else 'rgba(245,245,244,0.8)')};
    ...
    .proto-divider {{
        color: {('rgba(148,163,184,0.35)' if is_dark else 'rgba(0,0,0,0.15)')};
        font-size: 18px; line-height: 1; user-select: none; padding: 0 2px;
    }}
    .sf-pill {{
        ...
        background: {('rgba(13,91,140,0.15)' if is_dark else 'rgba(41,181,232,0.06)')};
        color: {('rgba(165,228,255,0.5)' if is_dark else 'rgba(13,91,140,0.4)')};
        border: 1px solid transparent;
        transition: all 0.4s ease;
        cursor: default;
    }}
    .sf-pill.active {{
        background: {('rgba(41,181,232,0.18)' if is_dark else 'rgba(41,181,232,0.12)')};
        color: #29B5E8;
        border-color: rgba(41,181,232,0.3);
        box-shadow: 0 0 12px rgba(41,181,232,0.15);
    }}
    .sf-pill .proto-dot {{
        width: 6px; height: 6px; border-radius: 50%;
        background: {('rgba(165,228,255,0.3)' if is_dark else 'rgba(13,91,140,0.25)')};
        transition: all 0.4s ease;
    }}
    .sf-pill.active .proto-dot {{
        background: #29B5E8;
        box-shadow: 0 0 6px rgba(41,181,232,0.5);
    }}
    .industry-pill {{
        display: flex; align-items: center; gap: 6px;
        padding: 5px 14px; border-radius: 20px;
        font-size: 11px; font-weight: 700; letter-spacing: 0.5px;
        text-transform: uppercase;
        background: {('rgba(30,41,59,0.5)' if is_dark else 'rgba(0,0,0,0.03)')};
    ...
    .industry-pill.active {{
        background: {('rgba(251,191,36,0.12)' if is_dark else 'rgba(217,119,6,0.08)')};
        color: {('#FBBF24' if is_dark else '#B45309')};
        border-color: {('rgba(251,191,36,0.3)' if is_dark else 'rgba(217,119,6,0.2)')};
        box-shadow: 0 0 12px {('rgba(251,191,36,0.1)' if is_dark else 'rgba(217,119,6,0.08)')};
    }}
    .industry-pill .proto-dot {{
        width: 6px; height: 6px; border-radius: 50%;
        background: var(--text-faint);
        transition: all 0.4s ease;
    }}
    .industry-pill.active .proto-dot {{
        background: {('#FBBF24' if is_dark else '#B45309')};
        box-shadow: 0 0 6px {('rgba(251,191,36,0.4)' if is_dark else 'rgba(217,119,6,0.3)')};
    }}
    ...
    .header-pill svg {{ width: 15px; height: 15px; stroke: var(--primary); }}
    ...
    .card {{
        ...
        border-radius: 16px;
        overflow: hidden;
        box-shadow: 0 2px 12px rgba(0,0,0,{('0.15' if is_dark else '0.03')});
        backdrop-filter: blur(12px);
        transition: all 0.3s ease;
    }}
    ...
    .welcome-banner {{
        background: {('linear-gradient(135deg, rgba(232,90,79,0.08) 0%, rgba(232,90,79,0.03) 100%)' if is_dark else 'linear-gradient(135deg, var(--primary-light) 0%, #FFF0EE 100%)')};
        border: 1px solid {('rgba(232,90,79,0.15)' if is_dark else 'transparent')};
    ...
    .welcome-icon {{
        ...
        font-size: 22px; box-shadow: 0 4px 12px rgba(232,90,79,0.25);
    }}
    .welcome-content h3 {{
    ...
    .welcome-content strong {{ color: var(--primary); font-weight: 700; }}
    ...
    .skeleton-img {{
        ...
        background: linear-gradient(90deg, var(--surface) 25%, {('rgba(51,65,85,0.5)' if is_dark else 'rgba(0,0,0,0.04)')} 50%, var(--surface) 75%);
        background-size: 200% 100%;
        animation: shimmer 1.5s infinite; margin-bottom: 14px;
    }}
    .skeleton-line {{
    ...
    /* ── AI Thinking Panel ── */
    .thinking-panel {{
        background: {('#0D1117' if is_dark else '#1E293B')};
        border: 1px solid {('rgba(52,211,153,0.2)' if is_dark else 'rgba(52,211,153,0.3)')};
    ...
    .thinking-line {{
        ...
        opacity: 0; animation: bootFadeIn 0.3s ease forwards;
    }}
    .thinking-line .hl {{ color: #34D399; font-weight: 600; }}
    .thinking-line .val {{ color: #FBBF24; }}
    ...
    .product-card:hover {{
        ...
        box-shadow: 0 8px 32px {hover_glow};
    ...
    .product-location svg {{ width: 13px; height: 13px; }}
    ...
    .stock-badge.out-stock {{ background: var(--danger-light); color: var(--danger); }}

    .product-visual {{
        width: 100%; height: 180px;
        background: {('linear-gradient(135deg, #1E293B, #0F172A)' if is_dark else 'linear-gradient(135deg, #F5F5F4, #FFFBF8)')};
        border-radius: 12px;
        display: flex; align-items: center; justify-content: center;
    ...
    .wishlist-heart {{
        ...
        background: {('rgba(0,0,0,0.4)' if is_dark else 'rgba(255,255,255,0.85)')};
        backdrop-filter: blur(8px);
        display: flex; align-items: center; justify-content: center;
    ...
    .wishlist-heart:hover {{
        background: rgba(232,90,79,0.2); transform: scale(1.15);
    }}
    ...
    .product-ribbon.bestseller {{ background: linear-gradient(135deg, #F59E0B, #D97706); }}
    .product-ribbon.new-item {{ background: linear-gradient(135deg, #8B5CF6, #6D28D9); }}
    .product-ribbon.on-sale {{ background: linear-gradient(135deg, #EF4444, #DC2626); }}
    ...
    .product-tag {{
        ...
        color: {('#C4A882' if is_dark else 'var(--accent)')};
    ...
    .price-original {{
        font-size: 14px; color: var(--text-faint); text-decoration: line-through;
    }}
    .discount-tag {{
        background: var(--primary-light); color: var(--primary);
    ...
    .fulfillment-info {{
        ...
        border-radius: 10px; font-size: 12px; color: var(--success);
        font-weight: 500; margin-bottom: 14px;
    }}
    .fulfillment-info svg {{ width: 15px; height: 15px; stroke: var(--success); }}
    ...
    .agent-panel-header {{
        padding: 18px;
        border-bottom: 1px solid var(--glass-border);
        background: {('linear-gradient(135deg, rgba(232,90,79,0.06) 0%, rgba(232,90,79,0.02) 100%)' if is_dark else 'linear-gradient(135deg, var(--primary-light) 0%, #FFF0EE 100%)')};
    }}
    .agent-panel-title {{
    ...
    .agent-status-text {{ font-size: 11px; color: var(--text-muted); }}
    ...
    .agent-badge.waiting {{ background: var(--surface); color: var(--text-faint); }}
    .agent-badge.active {{
        background: var(--primary-light); color: var(--primary);
        animation: pulse 1.5s infinite;
    }}
    .agent-badge.done {{ background: var(--success-light); color: var(--success); }}
    ...
    .activity-list {{ max-height: 300px; overflow-y: auto; padding: 14px; }}
    .activity-item {{
    ...
    .ai-insight-icon {{
        width: 24px; height: 24px; background: var(--primary-gradient);
    ...
    .ai-insight-tags {{ display: flex; gap: 8px; flex-wrap: wrap; }}
    .ai-tag {{
        background: {('rgba(30,41,59,0.6)' if is_dark else 'white')};
    ...
    .footer-text svg {{ width: 16px; height: 16px; }}
    .snowflake-badge {{
        display: inline-flex; align-items: center; gap: 6px;
        background: {('rgba(41,181,232,0.1)' if is_dark else '#E0F7FA')};
        color: #29B5E8; padding: 4px 12px; border-radius: 20px;
    ...
    .stTextInput input:focus {{
        border-color: var(--primary) !important;
        box-shadow: 0 0 0 3px rgba(232,90,79,0.15), 0 4px 20px rgba(232,90,79,0.1) !important;
    }}
    ...
    .stSelectbox label, .stTextInput label {{ color: var(--text) !important; }}

    hr {{ display: none !important; }}
    
    /* ── Dialogs ── */
    [data-testid="stDialog"] > div:first-child {{
        background: {('#131B2E' if is_dark else '#FEFDFB')} !important;
        border: 1px solid var(--glass-border) !important;
        border-radius: 20px !important;
        box-shadow: 0 20px 60px rgba(0,0,0,{('0.5' if is_dark else '0.15')}) !important;
        backdrop-filter: blur(20px);
        color: var(--text) !important;
    }}
    ...
    [data-testid="stModal"] > div {{ background: {('#131B2E' if is_dark else '#FEFDFB')} !important; }}
    div[data-modal-container="true"] > div {{ background: {('#131B2E' if is_dark else '#FEFDFB')} !important; }}
    [role="dialog"] {{ background: {('#131B2E' if is_dark else '#FEFDFB')} !important; }}
    [role="dialog"] > div {{ background: {('#131B2E' if is_dark else '#FEFDFB')} !important; }}

    /* ── Completion Card ── */
    .completion-card {{
        background: {('linear-gradient(135deg, rgba(52,211,153,0.15) 0%, rgba(52,211,153,0.08) 100%)' if is_dark else 'linear-gradient(135deg, var(--success-light) 0%, #C6F6D5 100%)')};
        border: 2px solid var(--success);
        border-radius: 16px; padding: 28px; text-align: center;
        margin-top: 28px; backdrop-filter: blur(8px);
        box-shadow: {('0 0 24px rgba(52,211,153,0.15), inset 0 0 20px rgba(52,211,153,0.05)' if is_dark else '0 4px 12px rgba(5,150,105,0.1)')};
    }}
    .completion-icon {{ font-size: 44px; margin-bottom: 14px; color: #34D399; }}
    .completion-title {{
        font-size: 20px; font-weight: 700;
        color: {('#34D399' if is_dark else '#065F46')}; margin-bottom: 6px;
    }}
    .completion-subtitle {{
        font-size: 14px; color: {('#6EE7B7' if is_dark else '#047857')};
    }}
    ...
    .ai-recommendation {{
        display: flex; align-items: center; gap: 8px;
        padding: 10px 14px;
        background: {('linear-gradient(135deg, rgba(251,191,36,0.1) 0%, rgba(251,191,36,0.05) 100%)' if is_dark else 'linear-gradient(135deg, #FEF3C7 0%, #FDE68A 100%)')};
        border-radius: 10px; margin-bottom: 12px;
        border: 1px solid {('rgba(251,191,36,0.3)' if is_dark else '#F59E0B')};
    }}
    .ai-rec-icon {{ font-size: 14px; }}
    .ai-rec-text {{
        font-size: 12px; font-weight: 600;
        color: {('#FBBF24' if is_dark else '#92400E')}; line-height: 1.3;
    }}

    .cortex-badge {{
        display: inline-flex; align-items: center; gap: 4px;
        background: {('rgba(41,181,232,0.1)' if is_dark else 'linear-gradient(135deg, #E0F2FE 0%, #BAE6FD 100%)')};
        color: {('#29B5E8' if is_dark else '#0369A1')};
    ...
    .agent-active-glow {{ animation: glowPulse 1.5s ease-in-out infinite; }}
    
    /* ── Toggle / Checkbox overrides ── */
    ...
    div[data-baseweb="radio"] label p {{ color: var(--text) !important; }}
    [data-testid="stRadio"] label p {{ color: var(--text) !important; }}
    [data-testid="stWidgetLabel"] p {{ color: var(--text) !important; }}
    [data-testid="stToggle"] label p {{ color: var(--text) !important; }}
    ...
    body.mobile-mode-active [data-testid="stMain"] {{
        ...
        max-width: 390px;
        margin: 40px auto;
        box-shadow: 0 0 0 12px #0F172A, 0 25px 80px rgba(0,0,0,0.5);
    ...
    body.mobile-mode-active .brand-title {{ font-size: 14px; }}
    body.mobile-mode-active .brand-subtitle {{ font-size: 10px; }}
    body.mobile-mode-active .header-pill {{ display: none; }}
    body.mobile-mode-active .protocol-strip {{
        margin: 0 -8px; padding: 6px 10px; gap: 4px; flex-wrap: wrap;
    }}
    body.mobile-mode-active .sf-pill,
    body.mobile-mode-active .industry-pill {{
        font-size: 9px; padding: 3px 8px;
    }}
    body.mobile-mode-active .proto-divider {{
        font-size: 14px;
    }}
    body.mobile-mode-active .proto-group {{
        gap: 4px;
    }}
    ...
    body.mobile-mode-active .user-name {{ font-size: 10px; }}
    body.mobile-mode-active .user-tier {{ font-size: 8px; }}
    body.mobile-mode-active .welcome-banner {{
        padding: 10px 12px; margin-bottom: 8px;
    }}
    body.mobile-mode-active .welcome-banner h3 {{ font-size: 12px; }}
    body.mobile-mode-active .welcome-banner p {{ font-size: 10px; }}
    body.mobile-mode-active .product-card {{ padding: 10px; }}
    body.mobile-mode-active .product-visual {{ height: 120px; }}
    body.mobile-mode-active .product-name {{ font-size: 12px; }}
    body.mobile-mode-active .price-current {{ font-size: 16px; }}
    body.mobile-mode-active .product-meta {{ font-size: 10px; }}
    body.mobile-mode-active .ai-recommendation {{ padding: 6px 10px; }}
    body.mobile-mode-active .ai-rec-text {{ font-size: 10px; }}
    body.mobile-mode-active .star-rating {{ margin-bottom: 4px; }}
    body.mobile-mode-active .star-rating .stars {{ font-size: 10px; }}
    body.mobile-mode-active .star-rating .rating-text {{ font-size: 9px; }}
    body.mobile-mode-active .agent-panel,
    body.mobile-mode-active .activity-panel {{
        display: none !important;
    }}
    body.mobile-mode-active .card-title {{ font-size: 12px; }}
    body.mobile-mode-active .card-header {{ padding: 10px 14px; }}
    body.mobile-mode-active .ai-insight-card {{ padding: 10px; }}
    body.mobile-mode-active .footer-bar {{ display: none; }}
    body.mobile-mode-active .floating-badge {{ display: none; }}
    body.mobile-mode-active .thinking-panel {{ padding: 12px 14px; }}
    body.mobile-mode-active .thinking-line {{ font-size: 10px; }}
    ...
    .mobile-nav-item {{
        ...
        gap: 4px; font-size: 10px; color: var(--text-faint);
    }}
...
import hashlib
def get_product_rating(sku):
    h = int(hashlib.md5(sku.encode()).hexdigest()[:8], 16)
    return round(4.0 + (h % 10) / 10.0, 1)

def get_product_reviews(sku):
    h = int(hashlib.md5(sku.encode()).hexdigest()[8:16], 16)
    return 50 + (h % 950)

def get_product_ribbon(product, idx):
    if product.get('base_price', 0) > 300:
        return "bestseller", "Bestseller"
    h = int(hashlib.md5(product.get('sku', str(idx)).encode()).hexdigest()[:4], 16)
    if h % 3 == 0:
        return "new-item", "New"
    discount = (product.get('base_price', 0) - product.get('current_price', 0)) / max(product.get('base_price', 1), 1)
    if discount > 0.1:
        return "on-sale", "Sale"
    return None, None
...
def get_snowflake_protocols(flow_step, sf_connected):
    """Snowflake Cortex services — what the app actually uses."""
    return {
        "Cortex Search": {"active": flow_step >= 1, "full": "Semantic Product Discovery", "icon": "❄"},
        "Cortex Complete": {"active": flow_step >= 1, "full": "LLM Agent Reasoning", "icon": "❄"},
        "Cortex Analyst": {"active": sf_connected, "full": "Text-to-SQL Analytics", "icon": "❄"},
        "MCP": {"active": flow_step >= 1, "full": "Model Context Protocol — Snowflake Native", "icon": "❄"},
    }

def get_industry_protocols(flow_step):
    """Industry commerce protocols the flow maps to."""
    return {
        "UCP": {"active": flow_step >= 0, "full": "Universal Commerce Protocol", "origin": "Google"},
        "A2A": {"active": flow_step >= 1, "full": "Agent-to-Agent Protocol", "origin": "Google"},
        "AP2": {"active": flow_step >= 3, "full": "Agent Payment Protocol", "origin": ""},
        "ACP": {"active": flow_step >= 3, "full": "Agent Commerce Protocol", "origin": "OpenAI"},
    }

# ── Boot Sequence ──
if not st.session_state.boot_complete:
    boot_step = st.session_state.boot_step
    if boot_step == 0:
        st.markdown("""
        <div class="boot-overlay">
            <div class="boot-logo">NOVAMART</div>
            <div class="boot-line" style="animation-delay: 0.6s;">Initializing 7 AI Agents...</div>
            <div class="boot-line" style="animation-delay: 1.2s;">Connecting to Snowflake Cortex...</div>
            <div class="boot-line" style="animation-delay: 1.8s;">Loading Retail Intelligence Platform...</div>
            <div class="boot-line ready" style="animation-delay: 2.4s;">Ready</div>
...
sf_protocols = get_snowflake_protocols(st.session_state.flow_step, sf_connected)
industry_protocols = get_industry_protocols(st.session_state.flow_step)

sf_pills = ""
for abbr, info in sf_protocols.items():
    active_class = "active" if info["active"] else ""
    sf_pills += f'<div class="sf-pill {active_class}" title="{info["full"]}"><span class="proto-dot"></span>{abbr}</div>'

industry_pills = ""
for abbr, info in industry_protocols.items():
    active_class = "active" if info["active"] else ""
    origin_tip = f' ({info["origin"]})' if info.get("origin") else ""
    industry_pills += f'<div class="industry-pill {active_class}" title="{info["full"]}{origin_tip}"><span class="proto-dot"></span>{abbr}</div>'

protocol_pills = f'<div class="proto-group">{sf_pills}</div><span class="proto-divider">·</span><div class="proto-group">{industry_pills}</div>'
...
if st.session_state.device_mode == "mobile":
    st.markdown("""
    <script>document.body.classList.add('mobile-mode-active');</script>
    <style>
        .block-container {
            max-width: 390px !important;
            margin: 0 auto !important;
...
col_state, col_cart, col_theme, col_spacer = st.columns([1, 0.6, 0.6, 2.8])
with col_state:
...
with col_left:
    ...
    if query and not st.session_state.pending_action and query != st.session_state.last_search_query:
        st.session_state.pending_action = "search"
    ...
    if st.session_state.current_products:
        ...
        for idx, product in enumerate(st.session_state.current_products):
            with prod_cols[idx % 2]:
                ...
                sku = product.get('sku', str(idx))
                rating = get_product_rating(sku)
                reviews = get_product_reviews(sku)
                stars_html = get_star_html(rating)
                ribbon_class, ribbon_text = get_product_ribbon(product, idx)
                ribbon_html = f'<div class="product-ribbon {ribbon_class}">{ribbon_text}</div>' if ribbon_class else ""
...
with col_right:
    ...
    done_count = len(st.session_state.completed_agents)
    progress_pct = int((done_count / 7) * 100)
    ...
    <div class="agent-panel">
        <div class="agent-panel-header">
            ...
            <div class="agent-counter">
                <span>{done_count}/7</span>
                <div class="agent-counter-bar"><div class="agent-counter-fill" style="width: {progress_pct}%;"></div></div>
            </div>
        </div>
...
if st.session_state.pending_action == "search":
    ...
    understanding_preview = agentic_query_understanding(query)
    occasion = understanding_preview.get("occasion", "")
    age_group = understanding_preview.get("age_group", "")
    categories = understanding_preview.get("product_categories", [])
    is_conv = understanding_preview.get("is_conversational", False)

    thinking_lines = [
        ('Analyzing query intent...', '', ''),
    ]
    if is_conv and occasion:
        thinking_lines.append(('Detected occasion:', 'hl', occasion))
    if is_conv and age_group:
        thinking_lines.append(('Age group identified:', 'hl', age_group))
    if is_conv and categories:
        thinking_lines.append(('Filtering categories:', 'val', ', '.join(categories)))
    thinking_lines.append(('Querying Cortex Search Service...', '', ''))

    lines_html = ""
    for i, (txt, cls, val) in enumerate(thinking_lines):
        if val:
            lines_html += f'<div class="thinking-line" style="animation-delay: {i*0.4}s;">{txt} <span class="{cls}">{val}</span></div>'
        else:
            lines_html += f'<div class="thinking-line" style="animation-delay: {i*0.4}s;">{txt}</div>'

    thinking_container.markdown(f"""
    <div class="thinking-panel">
        <div class="thinking-header">
            <span class="thinking-dot"></span>
            <span class="thinking-label">Cortex AI Reasoning</span>
        </div>
        {lines_html}
    </div>
    """, unsafe_allow_html=True)
    ...
    <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 18px;">
        <div class="skeleton-card"><div class="skeleton-img"></div><div class="skeleton-line medium"></div><div class="skeleton-line short"></div><div class="skeleton-line"></div></div>
        <div class="skeleton-card"><div class="skeleton-img"></div><div class="skeleton-line medium"></div><div class="skeleton-line short"></div><div class="skeleton-line"></div></div>
        <div class="skeleton-card"><div class="skeleton-img"></div><div class="skeleton-line medium"></div><div class="skeleton-line short"></div><div class="skeleton-line"></div></div>
        <div class="skeleton-card"><div class="skeleton-img"></div><div class="skeleton-line medium"></div><div class="skeleton-line short"></div><div class="skeleton-line"></div></div>
    </div>
    ...
    for agent_id, icon, _ in agent_order[:4]:
        ...
        add_activity(icon, f"{AGENT_DEFINITIONS[agent_id]['name']}{cortex_label}", "var(--surface)")
        st.session_state.completed_agents.add(agent_id)

    st.session_state.active_agent = None
    matched_products, understanding = search_products(query, limit=4)

    found_line = f'<div class="thinking-line" style="animation-delay: {len(thinking_lines)*0.4}s;"><span class="hl">Found {len(matched_products)} relevant products</span></div>'
    thinking_container.markdown(f"""
    <div class="thinking-panel">
        <div class="thinking-header">
            <span class="thinking-dot"></span>
    ...
    def order_completion_dialog():
        ...
        confetti_pieces = ""
        colors = ["#E85A4F", "#FBBF24", "#34D399", "#8B5CF6", "#29B5E8", "#F472B6", "#FB923C"]
        for i in range(40):
            color = colors[i % len(colors)]
            left = random.randint(0, 100)
            delay = random.uniform(0, 2)
            size = random.randint(6, 12)
            confetti_pieces += f'<div class="confetti-piece" style="left:{left}%;background:{color};width:{size}px;height:{size}px;animation-delay:{delay}s;border-radius:{random.choice(["50%","2px"])}"></div>'

        dc_map = AUSTRALIAN_STATES.get(st.session_state.selected_state, AUSTRALIAN_STATES["VIC"])
        state_abbr = st.session_state.selected_state
        dc_positions = {"NSW": (78, 62), "VIC": (72, 75), "QLD": (80, 38), "WA": (22, 52), "SA": (55, 60)}
        store_positions = {"NSW": (82, 58), "VIC": (76, 72), "QLD": (84, 34), "WA": (18, 48), "SA": (59, 56)}
        dc_x, dc_y = dc_positions.get(state_abbr, (72, 75))
        st_x, st_y = store_positions.get(state_abbr, (76, 72))

        st.markdown(f"""
        <div class="confetti-container">{confetti_pieces}</div>
        <div style="text-align: center; padding: 20px 0;">
            <div style="font-size: 48px; margin-bottom: 16px; color: #34D399;">✓</div>
            <div style="font-size: 20px; font-weight: 600; color: var(--success); margin-bottom: 4px;">Order {order_id}</div>
            <div style="color: var(--text-muted); font-size: 14px;">{cart_count} item(s) · ${cart_total:.2f}</div>
        </div>
        <div class="map-container">
            <div class="map-title">Supply Chain Route</div>
            <svg viewBox="0 0 100 100" width="240" height="180">
                <path d="M70,80 Q68,85 65,82 Q60,78 55,75 Q50,72 48,68 Q45,62 42,58 Q38,52 35,48 Q30,42 28,38 Q25,32 22,28 Q20,25 18,20 L22,15 Q28,12 35,10 Q45,8 55,10 Q65,12 72,18 Q78,22 82,28 Q85,35 86,42 Q87,50 85,58 Q83,65 80,72 Q78,78 75,82 Z"
                    fill="none" stroke="{'#334155' if is_dark else '#D6D3D1'}" stroke-width="0.8"/>
                <circle cx="{dc_x}" cy="{dc_y}" r="3" fill="#E85A4F" opacity="0.8">
                    <animate attributeName="r" values="3;5;3" dur="1.5s" repeatCount="indefinite"/>
                    <animate attributeName="opacity" values="0.8;1;0.8" dur="1.5s" repeatCount="indefinite"/>
                </circle>
                <circle cx="{st_x}" cy="{st_y}" r="2.5" fill="#34D399"/>
                <line x1="{dc_x}" y1="{dc_y}" x2="{st_x}" y2="{st_y}" stroke="#E85A4F" stroke-width="1" stroke-dasharray="3,2">
                    <animate attributeName="stroke-dashoffset" values="20;0" dur="2s" fill="freeze"/>
                </line>
                <text x="{dc_x}" y="{dc_y - 5}" text-anchor="middle" font-size="3.5" fill="{'#94A3B8' if is_dark else '#57534E'}">DC</text>
                <text x="{st_x}" y="{st_y - 4}" text-anchor="middle" font-size="3.5" fill="{'#94A3B8' if is_dark else '#57534E'}">Store</text>
            </svg>
            <div style="font-size: 11px; color: var(--text-faint); margin-top: 8px;">{dc_map['dc']} → {dc_map['stores'][0]}</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        for agent_id, icon, color, status in all_agents:
            st.session_state.completed_agents.add(agent_id)
            agent_row_bg = "rgba(52,211,153,0.1)" if is_dark else "#F0FDF4"
            agent_name_color = "#E2E8F0" if is_dark else "#1F2937"
            agent_status_color = "#94A3B8" if is_dark else "#6B7280"
            st.markdown(f"""
            <div style="display: flex; align-items: center; gap: 12px; padding: 8px 12px; background: {agent_row_bg}; border-radius: 8px; margin-bottom: 6px; border-left: 3px solid #059669;">
                <span style="font-size: 16px;">{icon}</span>
                <div style="flex: 1;">
                    <div style="font-size: 13px; font-weight: 500; color: {agent_name_color};">{AGENT_DEFINITIONS[agent_id]['name']}</div>
                    <div style="font-size: 11px; color: {agent_status_color};">{status}</div>
                </div>
...
<div class="footer-bar">
    <div class="footer-text">
        ...
        <span style="margin-left: 12px; color: var(--text-faint);">Cortex Search · Cortex Complete · Cortex Analyst</span>
    </div>
    <div class="footer-text" style="font-family: 'JetBrains Mono', monospace; font-size: 11px;">{stats_text}</div>
</div>
<div class="floating-badge">
    <span class="live-dot"></span>
    ❄️ Live on Snowflake Cortex
</div>
""", unsafe_allow_html=True)
